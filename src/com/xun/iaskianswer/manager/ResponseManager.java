...
public class ResponseManager {
	...
	public AbstractResponse productResponse(String turingResult, AnswerType type) {
		...
		case TEXT:
			...
			return textResponse;
		case URL:
			UrlResponse urlResponse = JSONUtil.objectFromJSONString(
					turingResult, UrlResponse.class);
			return urlResponse;
		case NOVEL:
			NovelResponse novelResponse = JSONUtil.objectFromJSONString(
					turingResult, NovelResponse.class);
			return novelResponse;
		case NEWS:
			NewsResponse newsResponse = JSONUtil.objectFromJSONString(
					turingResult, NewsResponse.class);
			return newsResponse;
		case APP:
			AppResponse appResponse = JSONUtil.objectFromJSONString(
					turingResult, AppResponse.class);
			return appResponse;
		case TRAIN:
			TrainResponse trainResponse = JSONUtil.objectFromJSONString(
					turingResult, TrainResponse.class);
			return trainResponse;
		case FLIGHT:
			FlightResponse flightResponse = JSONUtil.objectFromJSONString(
					turingResult, FlightResponse.class);
			return flightResponse;
		case GROUP:
			GroupResponse groupResponse = JSONUtil.objectFromJSONString(
					turingResult, GroupResponse.class);
			return groupResponse;
		case MOVIE:
			MovieResponse movieResponse = JSONUtil.objectFromJSONString(
					turingResult, MovieResponse.class);
			return movieResponse;
		case HOTEL:
			HotelResponse hotelResponse = JSONUtil.objectFromJSONString(
					turingResult, HotelResponse.class);
			return hotelResponse;
		case LOTTERY:
			LotteryResponse lotteryResponse = JSONUtil.objectFromJSONString(
					turingResult, LotteryResponse.class);
			return lotteryResponse;
		case PRICE:
			PriceResponse priceResponse = JSONUtil.objectFromJSONString(
					turingResult, PriceResponse.class);
			return priceResponse;
		case FOOD:
			FoodResponse foodResponse = JSONUtil.objectFromJSONString(
					turingResult, FoodResponse.class);
			return foodResponse;
		case KEY_LENGTH_ERROR:
			TextResponse keyLengthResponse = JSONUtil.objectFromJSONString(
					turingResult, TextResponse.class);
			return keyLengthResponse;
		case REQUEST_NULL:
			TextResponse requestNullResponse = JSONUtil.objectFromJSONString(
					turingResult, TextResponse.class);
			return requestNullResponse;
		case KEY_ERROR:
			TextResponse keyErrorResponse = JSONUtil.objectFromJSONString(
					turingResult, TextResponse.class);
			return keyErrorResponse;
		case NO_REQUEST_COUNT:
			TextResponse noRequstCountResponse = JSONUtil.objectFromJSONString(
					turingResult, TextResponse.class);
			return noRequstCountResponse;
		case NO_SUPPORT:
			TextResponse noSupportResponse = JSONUtil.objectFromJSONString(
					turingResult, TextResponse.class);
			return noSupportResponse;
		case UPDATING:
			TextResponse upDatingResponse = JSONUtil.objectFromJSONString(
					turingResult, TextResponse.class);
			return upDatingResponse;
		case FORMAT_ERROR:
			TextResponse formatErrorResponse = JSONUtil.objectFromJSONString(
					turingResult, TextResponse.class);
			return formatErrorResponse;
		default:
			TextResponse defaultResponse = JSONUtil.objectFromJSONString(
					turingResult, TextResponse.class);
			return defaultResponse;
		}
	}

	public void notifyViewPagerDataChanged(AbstractResponse mResponse,
			final Activity activity, List<View> list, TextView tv1,
			TextView tv2, TextView tv3, TextView tv4,
			MyPagerAdapter myPagerAdapte, NetworkImageView networkImageView,
			ImageView imageView) {
		if (mResponse instanceof TextResponse) {
			...
			if (list != null) {
				...
				for (int i = 0; i < mFlightResponse.list.size(); i++) {
					...
					View card_view = inflater.inflate(R.layout.card_result,
							null);
			...
			if (list != null) {
				...
				for (int i = 0; i < mMovieResponse.list.size(); i++) {
					...
					tv1 = (TextView) card_view.findViewById(R.id.textView1);
					tv2 = (TextView) card_view.findViewById(R.id.textView2);
					networkImageView = (NetworkImageView) card_view
							.findViewById(R.id.netWorkTestView);
					imageView = (ImageView) card_view
					...
					networkImageView.setImageUrl(
							mMovieResponse.list.get(i).icon, IAskIAnswerApp
